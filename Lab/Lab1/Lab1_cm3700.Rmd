...
# because the sample size is not big enough to approximate the ideal 0 and 1 condition.
...
#sample_means approach zero when sample size increases.
```
...
```{r}
normal1mil_mat_means<-colSums(normal1mil_mat)/100
```
...
```{r}
hist(normal1mil_mat_means)

#This histogram is close to a normal distribution, the sample mean is approching 3. 
```
