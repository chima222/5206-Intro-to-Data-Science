...
setwd("C:/Users/Machi/Desktop/5206/Lab")
...
```{r}
 ...
 KNN.decision <- function(Newpoint, K=5, Lagdata=Weekly[,2:6]) {
...
n.test<-nrow(test)
predications<-rep(NA,n.test)
for (i in 1:n.test){
  predications[i]<- KNN.decision(as.numeric(test[i,2:6]),K=5,train[,2:6])
  
}
test.error<-sum(predications!=test$Direction)/n.test
test.error
...
```{r}
for (i in 1:n.test){
  predications[i]<- KNN.decision(as.numeric(test[i,2:6]),K=3,train[,2:6])
...
```{r}
# ne KNN with direction variable
new.KNN.decision <- function(Newpoint, K=5, Lagdata=Weekly[,2:6], Direction = Weekly$Direction) {
...
fold.error<-rep(NA,k)
for(j in 1:k){
  new.test <- Weekly[fold == j,]
  new.train<- Weekly[fold != j,]
  
  new.n.test <- nrow(new.test)
  new.predictions <- rep(NA,new.n.test)
  for(i in 1:new.n.test){
    newp<- as.numeric(new.test[i,2:6])
    new.predictions[i]<-new.KNN.decision(newp,K=5,new.train[,2:6],new.train$Direction)
  }
  fold.error[j]<-mean(new.predictions!=new.test$Direction)
}
app.error<- mean(fold.error)
app.error
...
for(j in 1:k){
  ...
  for(i in 1:new.n.test){
    newp<- as.numeric(new.test[i,2:6])
    new.predictions[i]<-new.KNN.decision(newp,K=1,new.train[,2:6],new.train$Direction)
  }
  fold.error[j]<-mean(new.predictions!=new.test$Direction)
}
app.error<- mean(fold.error)
...
for(j in 1:k){
  ...
  for(i in 1:new.n.test){
    newp<- as.numeric(new.test[i,2:6])
    new.predictions[i]<-new.KNN.decision(newp,K=3,new.train[,2:6],new.train$Direction)
  }
  fold.error[j]<-mean(new.predictions!=new.test$Direction)
}
app.error<- mean(fold.error)
...
for(j in 1:k){
  ...
  for(i in 1:new.n.test){
    newp<- as.numeric(new.test[i,2:6])
    new.predictions[i]<-new.KNN.decision(newp,K=7,new.train[,2:6],new.train$Direction)
  }
  fold.error[j]<-mean(new.predictions!=new.test$Direction)
}
app.error<- mean(fold.error)
